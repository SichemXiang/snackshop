    ...
    <el-table :data="tableData" stripe style="width: 100%">
      ...
      <el-table-column prop="wxAvatarUrl" label="头像" width="200">
        <template v-slot="scope">
          <img :src="getServerUrl()+'avatar/'+scope.row.wxAvatarUrl" width="50" height="50">
        </template>
      </el-table-column>
      <el-table-column prop="phone" label="电话号码" />
      <el-table-column prop="registerTime" label="注册日期" width="200"/>
      <el-table-column prop="lastLoginTime" label="最后登录日期" width="200"/>
      <el-table-column prop="swiper" label="禁用"  align="center">
        <template #default="scope">
          <el-switch v-model="scope.row.openId" @change="banWxUser(scope.row)"></el-switch>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
